...
from pyhrp.hrp import Dendrogram, build_tree, hrp, _compute_distance_matrix
...
def test_build_tree_with_three_assets() -> None:
    ...
    cor = pd.DataFrame(
        [[1.0, 0.9, 0.3], [0.9, 1.0, 0.4], [0.3, 0.4, 1.0]],
        index=["A", "B", "C"],
        columns=["A", "B", "C"]
    )
    ...
    prices = pd.DataFrame(
        {
            ...
            "B": [50, 51, 50, 52, 51, 53, 54, 53, 55, 56]
        },
        index=dates
    )
...
def test_build_tree_root_has_all_leaves() -> None:
    ...
    cor = pd.DataFrame(
        [[1.0, 0.5, 0.3, 0.2], [0.5, 1.0, 0.4, 0.3], 
         [0.3, 0.4, 1.0, 0.6], [0.2, 0.3, 0.6, 1.0]],
        index=["A", "B", "C", "D"],
        columns=["A", "B", "C", "D"]
    )
