...
from pyhrp.hrp import dist, hrp_feed, split
...
def test_hrp():
    ...
    alpha = split(v1, v2)
    nt.assert_approx_equal(alpha[0], 0.7714285714285715)
    nt.assert_approx_equal(v1*alpha[0], v2*alpha[1])


def test_split():
    v1 = 3
    v2 = 5
    x = split(v_left=v1, v_right=v2)
    nt.assert_allclose(x, np.array([5.0, 3.0])/8.0)
    nt.assert_approx_equal(x.sum(), 1.0)
